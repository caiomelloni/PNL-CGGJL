"""Modelos e validação das tabelas de nós e arestas."""
...
def serialize_attributes(attributes: dict[str, Any]) -> str:
    """Serializa somente atributos preenchidos, como 'chave=valor; chave=valor'."""
    parts = []
...
class Node:
    ...
    def __post_init__(self) -> None:
        ...
        if re.fullmatch(rf"{expected_prefix}\d+", self.node_id) is None:
            raise ValueError(f"node_id {self.node_id!r} does not match type {self.type!r}")
